...
function trustedReadyEvidence(dir, overrides = {}) {
  ...
  const event = {
    schema_version: 'design-execution-event/v2',
    event: 'gate_finished',
  ...
  const registryPath = join(hostDir, 'reviewer-registry.json');
  const registry = {
    schema_version: 'design-reviewer-registry/v1',
    registry_id: 'host-reviewers-v1',
    reviewers: [{ id: 'reviewer-1', role: 'independent-reviewer', status: 'active' }]
  };
  writeFileSync(registryPath, `${JSON.stringify(registry, null, 2)}\n`);
  const machinePath = join(hostDir, 'machine-attestation.json');
  const machine = {
    ...
    execution_event: { path: eventPath, sha256: sha(readFileSync(eventPath)) },
    render_profile: {
      path: join(dir, 'render-profile.json'),
      sha256: sha(readFileSync(join(dir, 'render-profile.json')))
    },
  ...
  const reviewer = {
    ...
    checks: reviewerIds.map((id) => ({
      id,
      status: 'passed',
      evidence_refs: [join(dir, 'evidence-contract.json')]
    })),
    findings: [],
    remaining_risks: [],
    non_claims: ['Host attestation is filesystem-bound, not cryptographically authenticated.'],
    ...overrides.reviewer
  };
  ...
  return {
    hostDir,
    options: {
      machineAttestationPath: machinePath,
      reviewerRegistryPath: registryPath,
      reviewerAttestationPath: reviewerPath
    }
  };
}
  ...
  writeJson(dir, 'render-profile.json', {
    ...
    profiles: renderSpec.viewports.map((width) => ({
      ...
      reduced_motion_evidence: {
        preference_matches: true,
        static_text_equivalent: true,
        visible_text_sha256: sha('visible artifact text'),
        sampled_ms: 120,
        observed_animation_count: 0,
...
test('artifact-authored all-passed sidecars cannot authorize ready delivery', async () => {
  ...
  try {
    ...
    assert.match(errors, /runner machine attestation requires an existing absolute host-supplied path/i);
    assert.match(errors, /host reviewer registry requires an existing absolute host-supplied path/i);
    assert.match(errors, /host reviewer attestation requires an existing absolute host-supplied path/i);
  ...
  for (const reviewer of [
    ...
    try {
      ...
      if (reviewer.reviewer_id === 'artifact-author-1') {
        assert.match(errors, /host reviewer must differ from the artifact author/i);
      } else if (reviewer.reviewer_id) {
        assert.match(errors, /reviewer is not active in the supplied registry/i);
      } else {
        assert.match(errors, /predates runner machine evidence/i);
      }
    } finally {
      rmSync(dir, { recursive: true, force: true });
      rmSync(trusted.hostDir, { recursive: true, force: true });
...
test('artifact digest and evidence validation reject symbolic links', async () => {
  const {
  ...
  try {
    symlinkSync('source.csv', join(dir, 'linked-source.csv'));
    assert.throws(
      () => computeArtifactDigest(dir),
      /artifact digest rejects symbolic link.*linked-source\.csv/i
    );
    assert.match(
      validateEvidenceContract(dir).join('\n'),
      /artifact digest failed.*symbolic link.*linked-source\.csv/i
    );
  } finally {
    rmSync(dir, { recursive: true, force: true });
  }
...
test('reduced-motion profile requires behavioral and static-text evidence', async () => {
  const { validateEvidenceContract } = await import('../scripts/validate-evidence-contract.mjs');
  const dir = await validFixture({ delivery: 'blocked' });
  try {
    const profile = readJson(dir, 'render-profile.json');
    profile.profiles[0].reduced_motion_evidence.preference_matches = false;
    profile.profiles[1].reduced_motion_evidence.static_text_equivalent = false;
    profile.profiles[2].reduced_motion_evidence.active_animation_count = 1;
    profile.profiles[2].reduced_motion_evidence.visible_text_sha256 = '';
    writeJson(dir, 'render-profile.json', profile);
    const errors = validateEvidenceContract(dir).join('\n');
    assert.match(errors, /1440 reduced-motion preference evidence must pass/i);
    assert.match(errors, /390 reduced-motion text must match the default state/i);
    assert.match(errors, /320 reduced-motion active_animation_count must be zero/i);
    assert.match(errors, /320 reduced-motion visible_text_sha256 is required/i);
  } finally { rmSync(dir, { recursive: true, force: true }); }
});
