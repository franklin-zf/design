...
export const automaticGateIds = new Set([
  ...
  'validate-component-usage', 'render-smoke', 'tweakable-smoke'
]);
...
const gateOrder = [
  ...
  'validate-component-usage', 'validate-summary-map', 'validate-claim-map',
  'validate-design-system-package',
...
export function validateExecutionRequest(raw) {
  ...
  if (raw.output_surface?.component_refs !== undefined) {
    if (!Array.isArray(raw.output_surface.component_refs)) {
      errors.push('output_surface.component_refs must be an array');
    } else {
      if (raw.output_surface.component_refs.length > 3) {
        errors.push('output_surface.component_refs may contain at most three items');
      }
      if (raw.output_surface.component_refs.some((value) => !isNonEmptyString(value))) {
        errors.push('output_surface.component_refs must contain non-empty strings');
      }
      if (new Set(raw.output_surface.component_refs).size
          !== raw.output_surface.component_refs.length) {
        errors.push('output_surface.component_refs must be unique');
      }
    }
  }
...
function normalizeRequest(raw) {
  ...
  if (request.output_surface.component_refs) {
    request.output_surface.component_refs = [
      ...request.output_surface.component_refs
    ].sort();
  }
...
export function compileExecutionPlan(rawRequest, options = {}) {
  ...
  if (componentIds.length) {
    if (!options.componentCatalogue) {
      throw new Error('component catalogue is required when component_refs are present');
    }
    componentResolution = resolveComponentSelection({
      catalogue: options.componentCatalogue,
      componentIds,
      templateId: template.id,
      artifactType: request.output_surface.artifact_type
    });
  }
  ...
  const componentGates = componentResolution
    ? componentResolution.selected.flatMap((component) => component.required_gate_ids)
    : [];
  for (const id of componentGates) {
    if (!automaticGateIds.has(id)) {
      throw new Error(`unknown gate in component catalogue: ${id}`);
    }
  }
  ...
  const gateIds = [...new Set([
    ...registered.filter((id) => id !== 'manual-reviewer-pass'),
    ...componentGates,
    ...supplemental
  ])]
  ...
  const payload = {
    ...
    ...(componentResolution ? { component_resolution: componentResolution } : {}),
    ...
    evidence_contract: {
      ...
      ...(componentResolution ? { component_binding_required: true } : {})
    },
    runner: {
      max_concurrency: 2,
      cache: { mode: 'off' },
...
if (process.argv[1] && resolve(process.argv[1]) === modulePath) {
  try {
    ...
    const componentCatalogue = loadComponentCatalogue(
      ...
      resolve(packageRoot, 'schemas/component-catalogue.schema.json')
    );
