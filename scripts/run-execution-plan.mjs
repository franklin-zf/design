...
const gateAdapters = new Map([
  ...
  ['validate-component-usage', { script: 'validate-component-usage.mjs', target: 'artifact' }],
...
function validateCanonicalPlan(plan) {
  ...
  if (plan.runner.cache.mode !== 'off'
      && plan.runner.cache_capability === 'not_implemented') {
    throw new Error(
      'cache mode enabled is unsupported while cache_capability is not_implemented'
    );
  }
  ...
  const componentResolution = plan.component_resolution;
  const artifactComponentIds = [...(plan.artifact.component_refs || [])].sort();
  if (!componentResolution && artifactComponentIds.length) {
    throw new Error('component_refs require a component resolution');
  }
  if (componentResolution) {
    const selectedIds = componentResolution.selected
      .map((component) => component.component_id)
      .sort();
    if (sha256Value(selectedIds) !== sha256Value(artifactComponentIds)) {
      throw new Error('artifact component_refs do not match component resolution');
    }
    if (componentResolution.selected.some((component) => (
      component.technical_class === 'native_react'
    ))) {
      throw new Error('native_react cannot run in the shared-skill runner');
    }
    const gateIds = new Set(plan.automatic_gates.map((gate) => gate.gate_id));
    for (const component of componentResolution.selected) {
      for (const gateId of component.required_gate_ids) {
        if (!gateIds.has(gateId)) {
          throw new Error(`component gate is missing from plan: ${gateId}`);
        }
      }
    }
    if (!gateIds.has('validate-component-usage')) {
      throw new Error('component resolution requires validate-component-usage');
    }
  }
}
...
export function gateInvocation(
  ...
  if (gate.gate_id === 'validate-component-usage') {
    if (!componentResolutionPath) {
      throw new Error('validate-component-usage requires a resolution sidecar');
    }
  ...
  if (gate.gate_id === 'validate-evidence-contract') {
    ...
    if (trustedReadyEvidence.reviewerRegistryPath) {
      args.push(`--reviewer-registry=${trustedReadyEvidence.reviewerRegistryPath}`);
    }
    if (trustedReadyEvidence.reviewerAttestationPath) {
      args.push(`--reviewer-attestation=${trustedReadyEvidence.reviewerAttestationPath}`);
    }
  }
...
export async function runExecutionPlan(plan, options = {}) {
  ...
  if (options.cacheDir !== undefined) {
    throw new Error('cacheDir is unavailable while cache mode is off');
  }
  const trustDir = prepareWritablePath(
    options.trustDir || join(context.workspaceRoot, '.design-trust', plan.plan_id),
    context.workspaceRoot,
    'trustDir',
    'directory'
  );
  if (isInside(context.artifactRoot, trustDir)) {
    throw new Error('trustDir must be outside the artifact');
  }
  const telemetryPath = prepareWritablePath(
    options.telemetryPath || join(trustDir, 'execution.ndjson'),
    context.workspaceRoot,
    'telemetryPath',
    'file'
  );
  ...
  emit('run_started', {
    workers,
    cache_capability: plan.runner.cache_capability,
    cache_declared_mode: plan.runner.cache.mode,
    cache_effective_mode: 'off',
  ...
  let componentResolutionPath = null;
  if (plan.component_resolution) {
    componentResolutionPath = prepareWritablePath(
      join(context.artifactRoot, '.design', 'component-resolution.json'),
      context.workspaceRoot,
      'componentResolutionPath',
      'file'
    );
    const runtimeComponentResolution = {
      ...plan.component_resolution,
      plan_digest: plan.resolved_gate_plan_digest,
      artifact_digest: digest
    };
    secureWriteFile(
      componentResolutionPath,
      `${JSON.stringify(runtimeComponentResolution, null, 2)}\n`
    );
  }
  let machineAttestationPath = options.machineAttestationPath || null;
  const executeGate = options.executeGate || (async (gate) => {
    const invocation = gateInvocation(
      gate,
      context.artifactRoot,
      digest,
      plan.resolved_gate_plan_digest,
      renderSpecPath,
      componentResolutionPath,
      {
  ...
  while (pending.size) {
    ...
    const batchResults = await Promise.all(batch.map(async (gate) => {
      emit('gate_started', { gate_id: gate.gate_id, artifact_digest: digest });
    ...
    for (const result of batchResults) {
      ...
      if (result.gate_id === 'render-smoke' && result.status === 'passed'
          && !machineAttestationPath
          && typeof options.artifactAuthorId === 'string'
          && options.artifactAuthorId.trim()) {
        const renderProfilePath = join(context.artifactRoot, 'render-profile.json');
        if (!existsSync(renderProfilePath) || lstatSync(renderProfilePath).isSymbolicLink()) {
          throw new Error('passed render-smoke did not produce a regular render profile');
        }
        const eventSnapshotPath = prepareWritablePath(
          join(trustDir, 'render-execution-events.ndjson'),
          context.workspaceRoot,
          'renderExecutionEventSnapshot',
          'file'
        );
        secureWriteFile(eventSnapshotPath, readFileSync(telemetryPath));
        machineAttestationPath = prepareWritablePath(
          join(trustDir, 'machine-attestation.json'),
          context.workspaceRoot,
          'machineAttestationPath',
          'file'
        );
        const attestation = {
          schema_version: 'design-machine-attestation/v1',
          producer: 'design-runner',
          produced_at: new Date().toISOString(),
          artifact_author_id: options.artifactAuthorId.trim(),
          artifact_digest: digest,
          resolved_plan_digest: plan.resolved_gate_plan_digest,
          execution_event: {
            path: eventSnapshotPath,
            sha256: hashFile(eventSnapshotPath)
          },
          render_profile: {
            path: renderProfilePath,
            sha256: hashFile(renderProfilePath)
          },
          checks: ['render-smoke', 'remote-policy-pre-send', 'reduced-motion']
            .map((id) => ({ id, status: 'passed' }))
        };
        secureWriteFile(machineAttestationPath, `${JSON.stringify(attestation, null, 2)}\n`);
      }
      results.push(result);
...
function parseCli(argv) {
  ...
  if (positional.length !== 1) {
    throw new Error('Usage: node scripts/run-execution-plan.mjs <resolved-plan.json> [--workers=2] [--telemetry=<path>] [--trust-dir=<path>] [--artifact-author-id=<id>] [--machine-attestation=<path>] [--reviewer-registry=<path>] [--reviewer-attestation=<path>]');
  }
  return {
    ...
    trustDir: argv.find((arg) => arg.startsWith('--trust-dir='))?.slice(12),
    artifactAuthorId: argv.find((arg) => arg.startsWith('--artifact-author-id='))?.slice(21),
    machineAttestationPath: argv.find((arg) => arg.startsWith('--machine-attestation='))?.slice(22),
    reviewerRegistryPath: argv.find((arg) => arg.startsWith('--reviewer-registry='))?.slice(20),
    reviewerAttestationPath: argv.find((arg) => arg.startsWith('--reviewer-attestation='))?.slice(23)
  };
}
