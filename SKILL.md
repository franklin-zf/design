...
2. Control process: choose the artifact shape first, then lock the contract, plan the layout, build from templates, and validate with the smallest checks that cover the risk.
...
2. Read `references/capability-map.md` when the user asks for broad, Open Design-like, PPT-like, or multi-output capability.
3. Read `references/multi-agent-protocol.md` when the work uses CEO/product/design/AI-engineer/reviewer agents or must run across multiple agent hosts.
...
8. Read `references/screenshot-ui-evidence.md` when the artifact uses screenshots, UI captures, evidence walls, or before/after redesigns.
9. Read `references/template-index.md` before selecting or modifying a bundled template.
10. Read `references/validation.md` before claiming the artifact is ready.
...
4. Plan the layout before writing HTML or slides. Decks need a slide plan; reports need a report spine; dashboards need a metric model; charts need a chart contract; visually ambitious artifacts need an aesthetic contract.
5. Build from templates or established local patterns. Use `assets/templates/` as seed HTML, then adapt to the specific evidence and audience.
6. Generate `manifest.json`, `index.html`, and `quality-report.md` for every HTML artifact. Decks also need `slide-plan.json`.
7. Run `node scripts/validate-design-output.mjs <artifact-dir>` on every generated artifact directory.
8. For source-backed reports, dashboards, chart frames, decks, and PPT handoffs, include `summary-map.json` for visible summaries and run `node scripts/validate-summary-map.mjs <artifact-dir>`.
9. For source-backed reports, dashboards, and chart frames, include `claim-map.json` with file-backed `evidence_quotes` for verified claims, then run `node scripts/validate-claim-map.mjs <artifact-dir>`.
10. Run `node scripts/capability-preflight.mjs` before claiming browser, export, Figma, live connector, or Open Design daemon coverage. Use `--require=browser_launch` before claiming a browser can actually launch in the current host.
11. Run `node scripts/render-smoke.mjs <artifact-dir>/index.html --viewports=desktop,mobile --strict-layout` when browser overflow/overlap risk matters.
12. Run `node scripts/tweakable-smoke.mjs <artifact-dir>/index.html` for tweakable artifacts with controls.
13. Run `node scripts/validate-design-skill.mjs .` after changing this skill package.
14. For package-level validation, run `npm test`; run `npm run test:strict` when browser smoke can launch.
15. Hand off with evidence used, validation performed, unresolved assumptions, and exact artifact paths.
