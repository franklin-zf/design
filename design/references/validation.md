...
node scripts/validate-evidence-contract.mjs <artifact-dir>
...
In the V2 evidence contract, all published accessibility and privacy check ids are mandatory: `failed` blocks and `not_applicable` is not an allowed status. Privacy findings record id, severity, open/resolved status, summary, and evidence references; any open privacy finding blocks `ready`. An approved ready reviewer record requires all seven mandatory checks to be `passed` and cannot contain an open major or blocking finding. `approved` never overrides contradictory check or finding status.
...
`render-smoke.mjs` is optional only outside a resolved-plan evidence contract and requires Playwright from the host environment. In V2 profile mode it consumes the plan-bound render spec, executes every declared setup/assertion and segment at 1440/390/320, measures strict layout and computed schematic disclosure, inventories remote requests, and hashes screenshots under `<artifact-dir>/qa/`. A missing state/segment, failed assertion, unauthorized request, or screenshot mismatch blocks evidence validation.
