...
The package machine-validates `data-provenance.json` with `validate-data-provenance.mjs`. Derived execution is admitted only by a v2 resolved plan: exact registered hashes are trusted, statically bounded local Node code is restricted to verified disposable isolation, and unsafe or unavailable controls are untrusted with zero spawn. Read and write allowlists are artifact-relative; the sandbox is default-deny; every artifact-tree symlink is blocking; cache/telemetry paths reject symlink components. A named limiter must resolve to a registered CPU/memory limiter implementation. This candidate currently registers none, so derived execution blocks honestly with `registered-resource-limiter-unavailable`. Reviewer approval and caller-supplied booleans never change execution admission.
